...
<head>
	...
	<title>Game Store | Singleplayer Games</title>
		...
		<section id="game-container">
			<section class="container">
				<ol class="breadcrumb pt-2">
					...
					<li class="breadcrumb-item active">Singleplayer Games</li>
				</ol>
				<h2>Singleplayer Games</h2>
				<section class="row pb-5">
					<?php
					$sgame_service = new SgamesService();

					foreach ($sgame_service->getAll() as $sgame) {
						...
						<section class="col-xs-12 col-sm-6 col-md-4 col-lg-3 col-xl-3 px-2 py-3">
							<section class="card product-card h-100">
								<img src=" <?php echo '/img/sgames/' . getImageName($sgame) . '.jpg' ?>"
								     alt="<?php getImageName($sgame); ?>"
								     title="<?php $sgame->getTitle(); ?>>"
								     class="card-img-top w-100">

								<section class="card-footer px-2">
									<p class="card-title"><?php echo $sgame->getTitle(); ?></p>
									<section class="float-start">
										<p class="card-text"><small>By: <?php echo $sgame->getCreator(); ?><br>
											Price: <?php echo $sgame->getSprice(); ?> €</small>
										</p>
									</section>
									<?php
									...
									<div class="dropdown">
										...
										<div class="dropdown-menu dropdown-start px-2">
											...
											foreach ($gameList_service->getListsByUserId($_SESSION['userId']) as $gameList) {
												?>
												<a style="cursor: pointer" onclick="addToList(<?php echo $sgame->getItemId(); ?>, <?php echo $gameList->getGameListId(); ?>);"
												   class="dropdown-item py-0"><?php echo $gameList->getName(); ?></a>
												<?php
											}
											?>
										</div>
